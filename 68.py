...
class PasswordFoundException(Exception):
    def __init__(self,s):
		...
		print(s)
	    raise KeyboardInterrupt 	# To stop parallel processing, instead of sys.exit(0)
			...
			for i2,s2 in enumerate(grammar[2]):
				for i3,s3 in enumerate(grammar[3]):
					for i4,s4 in enumerate(grammar[4]):
						...
						try:
							Parallel(n_jobs=-1)(delayed(attempt5)(w, pw5, s6, n5, i6) for i6,s6 in enumerate(grammar[5]))
						except Exception, e:
							traceback.print_exc()
